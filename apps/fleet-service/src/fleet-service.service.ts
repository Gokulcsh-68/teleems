...
export class FleetServiceService {
  ...
  async findAllVehicles(query: VehicleQueryDto, requestUser: any) {
    ...
    // Tenant Isolation
    if (!isPlatformAdmin) {
      queryBuilder.andWhere('vehicle.organisationId = :orgId', {
        orgId: requestUser.organisationId,
      });
    ...
    if (vType)
      queryBuilder.andWhere('vehicle.vehicle_type = :vType', { vType });

    if (registration_number) {
      queryBuilder.andWhere('vehicle.registration_number ILIKE :reg', {
  ...
  async registerVehicle(dto: CreateVehicleDto, requestUser: any) {
    ...
    if (!orgId) {
      throw new ConflictException(
        'Organisation ID is required for vehicle registration',
      );
    }

    // Check for duplicate registration
    const existing = await this.vehicleRepo.findOneBy({
      registration_number: dto.registration_number,
    });
    if (existing) {
      throw new ConflictException(
        `Vehicle with registration number '${dto.registration_number}' already exists`,
      );
    }
    ...
    if (currentCount >= (org.vehicle_capacity || 10)) {
      throw new ConflictException(
        `Organisation '${org.name}' has reached its vehicle capacity (${org.vehicle_capacity})`,
      );
    }
  ...
  async updateVehicle(id: string, dto: any, requestUser: any) {
    ...
    if (
      !isPlatformAdmin &&
      vehicle.organisationId !== requestUser.organisationId
    ) {
      throw new ForbiddenException(
        'Access denied: You can only update vehicles in your own organization',
      );
    }
  ...
  async deleteVehicle(id: string, requestUser: any) {
    ...
    if (
      ...
      throw new ForbiddenException(
        'Access denied: You can only delete vehicles in your own organization',
      );
    }
  ...
  async findOneOperator(id: string): Promise<FleetOperator> {
    ...
    if (!operator)
      throw new NotFoundException(`Fleet Operator with ID ${id} not found`);
    return operator;
  }

  async updateOperator(
    id: string,
    dto: UpdateFleetOperatorDto,
    adminId: string,
    ip: string,
  ): Promise<FleetOperator> {
