...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  constructor(
    ...
    const configuredTimeout = parseInt(config.get('ADMIN_SESSION_TIMEOUT_MINUTES') || '30', 10);
  ...
  async validate(payload: any) {
    ...
    if (payload.purpose) {
      return { userId: payload.sub, roles: payload.roles || [payload.role], purpose: payload.purpose };
    }
